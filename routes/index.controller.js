...
async function postHomePage (req, res, next) {
  try {
    ...
    figlet.text(`${zodiac}`, {
      ...
      console.log(ascii)
      return res.status(200).render('sign', { title: 'HOROSCOPE APP', name, sign: ascii, description: data.description, current_date: data.current_date, lucky_number: toAscii(zodiac) })
    })
  } catch (error) {
    console.error(error)
    next(error)
